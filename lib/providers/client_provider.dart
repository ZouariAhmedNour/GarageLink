  ...
  List<Client> filter({
    ...
    return state.clients.where((c) {
      final matchesNom =
          nom == null ||
          nom.isEmpty ||
          c.nomComplet.toLowerCase().contains(nom.toLowerCase());
      final matchesImmat =
          immat == null ||
          immat.isEmpty ||
          c.vehiculeIds.any(
            (vid) => vid.toLowerCase().contains(immat.toLowerCase()),
          );
      final matchesPeriode =
          true; // si tu veux filtrer par periode selon dateNaissance ou date de création tu peux adapter
