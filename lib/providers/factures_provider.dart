...
class FactureNotifier extends StateNotifier<FactureFilterState> {
  ...
  // listes / CRUD
  void setFactures(List<Facture> factures) => state = state.copyWith(factures: factures);
  void addFacture(Facture facture) => state = state.copyWith(factures: [facture, ...state.factures]);
  ...
  void removeFacture(String id) => state = state.copyWith(factures: state.factures.where((f) => f.id != id).toList());

  // filtres
  void setDateRange(DateTime? start, DateTime? end) => state = state.copyWith(start: start, end: end);
  void setClientFilter(String client) => state = state.copyWith(clientFilter: client);
  void setQuery(String query) => state = state.copyWith(query: query);

  // nouveau : set plage montant (min ou max peuvent être null)
  void setMontantRange(double? min, double? max) => state = state.copyWith(minMontant: min, maxMontant: max);

  // clear plage montant
  void clearMontantRange() => state = state.copyWith(minMontant: null, maxMontant: null);
  ...
  List<Facture> filtered({DateTime? start, DateTime? end}) {
    ...
    // Filtrage par période si définie
    if (startDate != null && endDate != null) {
      list = list.where((f) => !f.date.isBefore(startDate) && !f.date.isAfter(endDate)).toList();
    }

    // Filtrage par client (optionnel)
    if (state.clientFilter.isNotEmpty) {
      final pattern = state.clientFilter.toLowerCase();
      list = list.where((f) => f.clientName.toLowerCase().contains(pattern)).toList();
    }

    // Si on est en mode "période", on retourne la liste (période déjà appliquée)
    if (state.searchField == SearchField.periode) {
      list.sort((a, b) => b.date.compareTo(a.date));
      return list;
    }

    // Mode montant : si une plage min/max est définie, applique la plage
    if (state.searchField == SearchField.montant) {
      ...
      // si pas de plage définie mais une query textuelle, on essaie de parser la query (fallback)
      if (state.query.isNotEmpty) {
        ...
        if (parsed != null) {
          list = list.where((f) =>
              f.montant.toString().contains(qNumStr) ||
              f.montant.toStringAsFixed(2).contains(qNumStr) ||
              (f.montant - parsed).abs() < 0.0001
          ).toList();
        } else {
          list = list.where((f) => f.montant.toString().contains(q)).toList();
        }
        list.sort((a, b) => b.date.compareTo(a.date));
        return list;
      }
    ...
    // Autres modes (client / immatriculation / id) : utiliser query texte si présent
    if (state.query.isNotEmpty) {
      final q = state.query.trim().toLowerCase();
      switch (state.searchField) {
        case SearchField.client:
          list = list.where((f) => f.clientName.toLowerCase().contains(q)).toList();
          break;
        case SearchField.immatriculation:
          list = list.where((f) => (f.immatriculation ?? '').toLowerCase().contains(q)).toList();
          break;
        case SearchField.id:
          list = list.where((f) => f.id.toLowerCase().contains(q)).toList();
          break;
  ...
  double totalMontant({DateTime? start, DateTime? end}) {
    return filtered(start: start, end: end).fold(0.0, (sum, f) => sum + f.montant);
  }
}
